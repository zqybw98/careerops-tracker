...
def build_next_action_recommendation(
    ...
    status = normalize_status(classification.get("suggested_status"))
    ...
    if status == "Interview / Assessment" and category == "Interview Invitation":
    ...
    if status == "Interview / Assessment" or category == "Assessment / Coding Test":
    ...
    if status == "Rejected" or category == "Rejection":
        ...
        return {
            ...
            "rationale": f"Rejection email detected; close the loop and preserve review context.{reason_phrase}",
        }

    if status == "Waiting" or category in {"Application Confirmation", "Process Delay"}:
        ...
        return {
            ...
            "follow_up_date": follow_up_date,
            "template_type": "Follow-up Email",
            "rationale": "The email indicates confirmation or delay; no action is required now.",
        }

    if status == "Action Needed" or category in {"Recruiter Reply", "Follow-up Needed"}:
...
def build_workflow_decision(
    ...
    category = str(classification.get("category") or "Other")
    suggested_status = normalize_status(classification.get("suggested_status"))
    current_status = normalize_status(application.get("status")) if application else ""
    ...
    if suggested_status == "Interview / Assessment" and category == "Interview Invitation":
        return {
            "operation": "Prepare interview",
            "review_level": "Low" if auto_match else "Medium",
            "record_action": "Update status, store interview context, and schedule preparation.",
            "status_action": _status_action_text(current_status, "Interview / Assessment"),
    ...
    if suggested_status == "Interview / Assessment":
        return {
    ...
    if suggested_status == "Waiting":
        return {
            "operation": "Schedule follow-up",
            "review_level": "Low" if auto_match else "Medium",
            "record_action": "Update early-stage status and set a follow-up reminder.",
            "status_action": _status_action_text(current_status, "Waiting"),
            "primary_action_label": "Apply waiting update",
            "secondary_action_label": "Save wait note only",
            "status_update_allowed": True,
    ...
    if suggested_status == "Action Needed":
        return {
            "operation": "Reply required",
            "review_level": "Medium",
            "record_action": "Keep the record active and prepare a recruiter response.",
            "status_action": _status_action_text(current_status, "Action Needed"),
            "primary_action_label": "Apply action-needed update",
